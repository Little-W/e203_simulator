...
SIM_OPTIONS   +=  -Wno-PINCONNECTEMPTY -Wno-fatal -Wno-WIDTH -Wno-CASEINCOMPLETE -Wno-UNOPTFLAT --no-timing
...
SIM_CMD := ${SIM_EXEC} +DUMPWAVE=${DUMPWAVE} +TESTCASE=${PROGRAM} +SIM_TOOL=${SIM_TOOL} 2>&1 | tee ${SIM_OUT_DIR}.log
TEST_CMD := ${SIM_EXEC} +DUMPWAVE=${DUMPWAVE} +TESTCASE=${TEST_PROGRAM} +SIM_TOOL=${SIM_TOOL} 2>&1 | tee ${TEST_NAME}.log
...
compile.flg: ${RTL_V_FILES} ${TB_V_FILES}
	...
	${SIM_TOOL_EXEC} ${SIM_OPTIONS}  ${RTL_V_FILES} ${TB_V_FILES} ${TB_CC_FILE} ${SIM_OPTIONS_BACK}
